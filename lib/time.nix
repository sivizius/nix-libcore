{ context, debug, float, integer, intrinsics, list, string, type, ... }:
let
  debug'                                =   debug ( context ++ [ "time" ] );
  inherit (type)    isInteger isList isString matchOrFail maybeInteger maybeString;
  inherit (list)    fold generate get head;
  inherit (string)  match repeat split toLowerCase toUpperCase;
  ...
  =   {
        ...
        &&  maybeInteger  month
        &&  maybeInteger  day
        &&  maybeInteger  dayOfWeek
        &&  maybeInteger  dayOfYear
        &&  maybeInteger  hour
        &&  maybeInteger  minute
        &&  maybeInteger  second
        &&  maybeInteger  nanosecond
        &&  maybeInteger  zone
      ...
      format:
        let
          ...
          =   token:
                ...
                then
                  let
                    parts               =   match "%([-_0+^#])*([EO]?[A-Za-y%]|(:{0,3})z)" ( head token );
                    config
                    =   fold
                        (
                          state:
                    ...
                    =   value:
                        ...
                        default:
                          let
                            ...
                            text        =   if value != null then string value else repeat "?" length;
                            len         =   ( stringLength text ) - length;
                    ...
                    =   text:
                        maybeLower:
                          if      maybeLower && config.opposite   then  toLowerCase text
                          else if config.upper                    then  toUpperCase text
                          else                                          text;
                    control             =   get parts 1;
                    suffix              =   get parts 2;
                    suffix'             =   stringLength suffix;
        ...
        in
          ...
          then
            ...
            ""
            ( split "%[-_0+^#]*([EO]?[A-Za-y%]|:{0,3}z)" format )
          else
  ...
  tryParseISO8601#: string -> DateTime | null
    =   iso8601:
        ...
        in
          ...
          then
            {
              year                      =   toInteger' ( get list  0 );
              month                     =   toInteger' ( get list  2 );
              day                       =   toInteger' ( get list  4 );
              dayOfWeek                 =   null;
              dayOfYear                 =   null;
              hour                      =   toInteger' ( get list  6 );
              minute                    =   toInteger' ( get list  9 );
              second                    =   toInteger' ( get list 11 );
              nanosecond                =   0;
              zone                      =   toInteger' ( get list 13 );
              zoneName                  =   null;
            }
          else
            null;
  ...
  =   unix:
      zoneOrName:
        ...
        in
          { inherit unix year month day hour minute second nanosecond  dayOfWeek dayOfYear zone zoneName; };
in
{
  __functor = self: from;
  inherit daysPerYear secondsPerMinute secondsPerHour secondsPerDay yearsPerEra yearsPerCentury yearsPerCycle monthsPerYear;
  inherit DateTime;
  inherit after before from getMonthName;
  inherit format formatDate formatDateTime formatYearMonth;
  inherit parseDateTime parseISO8601 parseUnixTime;
  current                               =   intrinsics.currentTime or 0;
}
