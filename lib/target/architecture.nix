...
let
  debug'                                =   debug ( context ++ [ "architecture" ] );
  ...
  =   {
        ...
        =   bits:
            ...
            family:
            {
              __type__                  =   "Architecture";
              __toString                =   { name, ... }: name;
              inherit bits endianness family;
            };

        __functor#: type -> (string | Architecture)? -> Architecture
        =   let
              fail                      =   debug'.error "Architecture";
            in
              { ... }:
              architecture:
                type.matchOrFail architecture
                {
                  null                  =   all.unknown;
                  string                =   all.${architecture} or (fail "Unknown architecture »${architecture}«!");
                  set
                  =   if architecture.__type__ or null == "Architecture"
                      then
                        architecture
                      else
                        fail "Not an architecture!";
                };
      };
